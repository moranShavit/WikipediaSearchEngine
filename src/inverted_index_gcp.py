...
class MultiFileReader:
    ...
    def read(self, locs, n_bytes, is_text_posting=False):
        b = []
        for f_name, offset in locs:
            # f_name = str(self._base_dir / f_name) to uncomment and replace with the one under before upload to gcp
            if is_text_posting:
                f_name = (self._base_dir / f_name).as_posix() # work for posting_gcp but not for title index need to fix it to be dynamic
...
class InvertedIndex:  
    ...
    def read_a_posting_list(self, base_dir, w, bucket_name=None, is_text_posting=False):
        posting_list = []
        if not w in self.posting_locs:
            return posting_list
        with closing(MultiFileReader(base_dir, bucket_name)) as reader:
            locs = self.posting_locs[w]
            b = reader.read(locs, self.df[w] * TUPLE_SIZE, is_text_posting=is_text_posting)
            for i in range(self.df[w]):
                doc_id = int.from_bytes(b[i*TUPLE_SIZE:i*TUPLE_SIZE+4], 'big')
                tf = int.from_bytes(b[i*TUPLE_SIZE+4:(i+1)*TUPLE_SIZE], 'big')
                posting_list.append((doc_id, tf))
    ...
    def read_index(base_dir, name, bucket_name=None):
        # path = str(Path(base_dir) / f'{name}.pkl')  to uncomment and replace with the one under before upload to gcp
        path = (Path(base_dir) / f"{name}.pkl").as_posix()
        bucket = None if bucket_name is None else get_bucket(bucket_name)
